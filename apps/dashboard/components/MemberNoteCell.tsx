...
export function MemberNoteCell({
  ...
  if (!isAdmin) {
    return <div className={`truncate ${note ? "text-[#B7C0D0]" : "text-[#5C6577]"}`}>{note ?? "-"}</div>;
  }
  ...
  if (isEditing) {
    return (
      <input
        ...
        placeholder="비고"
        className="w-full rounded-md border border-[#4472C4]/50 bg-[#0F131B] px-1.5 py-1 text-[13.5px] text-[#E6EAF2] outline-none"
      />
    );
  }
  ...
  return (
    <button
      ...
      title="클릭해서 비고 수정"
      disabled={isPending}
      className={`truncate text-left hover:underline ${note ? "text-[#B7C0D0]" : "text-[#5C6577]"}`}
    >
      {isPending ? "저장 중..." : note ?? "-"}
