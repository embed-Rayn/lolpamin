...
export function NavLink({ href, label, icon, active, badge }: NavLinkProps) {
  return (
    <Link
      href={href}
      className={`flex items-center gap-2 rounded-lg py-1.5 pl-3 pr-2.5 text-[12.5px] transition-colors ${
        active ? "bg-[#20293A] text-white font-bold" : "text-[#95A0B2] font-medium hover:bg-[#1B2130] hover:text-[#E6EAF2]"
      }`}
    >
      <span className={`w-6 flex-none font-mono text-[10.5px] ${active ? "text-[#8FB4F5]" : "text-[#4E576A]"}`}>
        {icon}
      </span>
...
export function NavGroupLink({ href, number, label, active }: NavGroupLinkProps) {
  return (
    <Link
      href={href}
      className={`flex items-center gap-2 rounded-lg px-2 py-1.5 text-[12px] font-bold tracking-wide transition-colors ${
