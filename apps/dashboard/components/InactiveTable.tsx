...
export function InactiveTable({ rows }: { rows: InactiveRow[] }) {
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-white/[.06] bg-[#151A24]">
      ...
      <div className="grid grid-cols-[186px_164px_1fr_120px_146px] border-b border-white/[.06] bg-[#12161F] px-5 py-2.5 text-[12px] font-bold text-[#6E7889]">
        <div>실명</div>
        <div>카톡 닉네임</div>
        <div>경과일</div>
      ...
      {rows.map((r) => (
        <div key={r.id} className="grid grid-cols-[186px_164px_1fr_120px_146px] items-center border-b border-white/[.04] px-5 py-3 hover:bg-[#181E29]">
          ...
          <div className="text-right font-mono text-[12.5px] text-[#7A8496]">{r.lastActiveDate}</div>
