...
- 작성일: 2026-09-01
- 갱신: 2026-09-03 — ELO→MMR 이름변경 반영, 구현된 범위와 남긴 범위 표시
- 관련 스펙: `docs/superpowers/specs/2026-09-01-account-matching-and-reversible-linking-design.md`

## 구현 상태 (2026-09-03)

이 문서의 **경기 기록과 되돌리기는 구현되었다.** 회원 변경 로그(`MemberChangeLog`)는
아직이다 — 아래 「남긴 범위」 참고.

| 항목 | 상태 |
|---|---|
| 경기 기록 화면 (`/match-history`) | 구현 |
| 되돌리기 (`cancelGameResult`) | 구현 |
| 경기 입력·취소의 운영진 귀속 | 구현 |
| 취소된 경기를 내전 횟수에서 제외 | 구현 |
| 회원 변경 로그 (`MemberChangeLog`) | **미구현** |

### 남긴 범위

`MemberChangeLog`와 「변경 기록」 구획은 넣지 않았다. 요청받은 것이 경기 기록과
되돌리기였고, 회원 변경 로그는 새 테이블과 네 뮤테이션의 시그니처 변경을 함께 끌고 오는
독립적인 작업이라 따로 다루는 편이 낫다. 그래서 아래 「회원 변경: 새 테이블」과
「`/link-accounts` 아래 「변경 기록」」 절, 그리고 「운영진 귀속을 위한 시그니처 변경」의
네 뮤테이션 항목은 **아직 설계일 뿐 구현이 아니다.**

### 이름변경

이 문서는 ELO→MMR 이름변경(`20260902083425_rename_elo_to_mmr`) 전에 쓰였다. 본문의
`elo`·`eloBefore`·`eloAfter`는 현재 코드에서 각각 `mmr`·`mmrBefore`·`mmrAfter`다.

### 구현하며 달라진 것

- 내비게이션 번호: 뽑기 두 개가 들어와 「경기 기록」은 03이고 그 뒤가 한 칸씩 밀렸다
  (미활동 04, 카톡 05, 계정 연결 06, 대포 07, 핀볼 08, 관리자 09).
- `createdById`는 서버 액션이 세션에서 채운다. 클라이언트가 보낸 값은 쓰지 않는다 —
  그러지 않으면 아무나 남의 이름으로 입력 기록을 남길 수 있다.
- `GameResult`에 `@@index([cancelledAt, createdAt])`를 뒀다. 되돌리기 대상을 찾는 질의가
  정확히 이 순서다.
