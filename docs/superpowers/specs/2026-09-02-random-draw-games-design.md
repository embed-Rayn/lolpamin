...
## 설계 변경 3 (2026-09-02, 구현 중)

07 레이스가 너무 짧고 싱거워서 코스를 다시 짰다. 이전 코스는 한 화면(460유닛)에 핀 몇 줄뿐이라 2초면 끝났다.

- 코스를 **1900유닛(화면 3.6배)**으로 늘리고 화면은 선두를 따라 스크롤한다(`VIEW_HEIGHT` 520). 오른쪽 레일이 전체 낙하 구간에서 각자 어디쯤인지 보여주고, 상단에 「선두 ○○○ · NN%」가 뜬다. 선두 구슬은 금색 테두리.
- 구간: 핀 밭 → 벽면 편향판 → 회전 십자 2개 → 좁은 관문 3개(구멍 위치가 좌·우로 엇갈림) → 범퍼 군집(반발 1.2, 뒤로 튕겨나감) → 좌우 왕복 바 2개 → 마지막 깔때기 병목.
- 장애물은 **마찰 0**으로 둔다. 완만한 경사에 구슬이 얹혀 서버리면 레이스가 죽는데, 실제로 첫 시도에서 관문 벽에 전원이 얹혀 90초 상한까지 갔다. 속도는 마찰이 아니라 낙하 거리와 반사 횟수로 조절한다.
- 구간 간격은 **어떤 두 장애물도 구슬(지름 18)보다 좁은 쐐기를 만들지 않도록** 배치한다. 첫 시도의 지그재그 경사판 두 개가 8유닛 틈을 만들어 구슬을 물어버렸다.
- 시간 상한 90초. 실측 완주 시간은 8~15초대이고, 단위 테스트가 평균 8~45초 범위와 "선두가 최소 한 번은 바뀐다"를 검증한다.

## 설계 변경 4 (2026-09-02, 구현 중)

07 레이스를 20초로 늘리고 막판 뒤집기를 넣었다.

- 중력을 1 → 0.4로 낮춰 낙하 속도를 약 70%로 만들고, 코스를 1900 → 2740유닛으로 늘렸다. 실측 평균 20.6초(19.6~21.7초), 브라우저 실측 21.7초.
- 추가 구간: 두 번째 핀 밭, 세 번째 회전 십자 + 관문 하나, 그리고 골 직전 **회전 물레방아 2기**(날개 6장, 좌우 역회전). 구슬이 날개 사이에 담겨 한참 실려 갔다 뱉어지므로 다 잡은 1등이 골 앞에서 뒤집힌다 — 20판 중 8판에서 물레방아 진입 시점의 선두가 최종 승자와 달랐다.
- 좁은 관문(4번 구간)을 고쳤다. 구멍이 전부 좌·우로만 열려 있어 아무도 범퍼 군집으로 가지 않았다. 마지막 관문의 구멍을 범퍼 중앙 바로 위로 옮겨 8명 중 6~8명이 범퍼를 통과한다.
- 관문 벽을 직선 판 대신 **물결치는 짧은 조각들의 사슬**로 바꿨다(`wavyBarrier`). 조각마다 각도가 흔들려 튀는 방향을 예측할 수 없지만, 전체 기울기는 계속 구멍 쪽 내리막이라 구슬이 얹혀 서지 않는다.

## 설계 변경 5 (2026-09-02, 구현 중)

07의 범퍼를 5개 → **11개**(3줄 엇갈림 배치)로 늘리고 반발계수를 1.2 → **2.0**으로 올렸다.

- 반발 2.0은 받은 것보다 많이 돌려주므로 구슬이 범퍼 사이를 다시 위로 타고 오르기도 한다. 시뮬레이션에서 강한 상향 반동(속도 y < -6)이 판마다 0~6회 관측된다.
- 폭주하지는 않는다: 최대 속도 12.4에서 멎고, 8명 중 6~8명이 범퍼 구역을 통과하며 완주 시간은 여전히 상한 안쪽이다.
- 범퍼가 시간을 더 먹어 중력을 0.4 → 0.45로 되돌렸다. 평균 20.1초(16.5~21.4), 브라우저 실측 20.7초.

## 설계 변경 6 (2026-09-02, 구현 중)

**시간만 충분하면 전원이 완주하도록** 맵을 손봤다. 이전 맵은 8명 중 4~7명만 골인하고 나머지는 영영 걸려 있었다. 마찰이 0이라 오목한 곳이나 코너에 한 번 자리 잡으면 절대 빠져나오지 못한다. 걸린 지점과 원인:

- **물결 관문의 오목한 골** — 조각의 *높이*를 물결치게 해서 내려갔다 올라오는 구간이 생겼다. 이제 조각을 끝점끼리 이어 붙여(체인) 표면이 단조 하강만 하게 하고, 물결은 각도에만 준다.
- **좌우 벽 코너** — 핀 밭이 벽에서 16유닛까지 붙어 있어 벽과 핀 사이에 구슬(지름 18)이 끼는 주머니가 생겼다. 핀을 벽에서 44유닛 떨어뜨렸다.
- **수평 왕복 바** — 마찰 0인 평평한 바 위에 얹힌 구슬은 영원히 안 굴러떨어진다. 바를 ±0.16rad 기울였다.
- **물레방아 아래 능선** — 기울기 부호가 반대라 벽 쪽으로 배수되어 벽 코너에 주머니를 만들었다. 부호를 뒤집어 가운데로 흐르게 했다.

이제 8명 전원이 23~25초 안에 골인한다. 단위 테스트가 "시간이 충분하면 모든 구슬이 골에 도달한다"(120초 상한, 3판)를 검증한다.

## 설계 변경 7 (2026-09-02, 구현 중)

범퍼를 진짜 핀볼처럼 튕기게 했다.

- matter의 `restitution`은 1 부근에서 포화한다. 2로 두나 4로 두나 반동이 같았다(실측: 최대 상향속도 6.3~7.1로 동일, 범퍼 구역 상향 반동 0회). 그래서 **충돌 시 명시적 킥 임펄스**로 바꿨다: 범퍼에 닿은 구슬은 범퍼 중심에서 바깥으로 `BUMPER_KICK` 속도로 튕겨 나간다.
- 세기 스윕(전원 완주는 모두 유지): 8 → 상향 8.0·꼬리 38초, 12 → 12.0·29초, **16 → 17.6·31초**, 20 → 20.0·42초에 최대속도 31(두께 11 판을 뚫을 위험). **16**을 골랐다. 판당 범퍼 반동이 약 100회 발생한다.
- 범퍼는 `label: "bumper"`로 표시하고 렌더러가 주황(#E0603C, 테두리 #FFC48A)으로 그린다. 세게 튕기는 유일한 장애물이라 색으로 구분할 값어치가 있다.
